...
func InitGorm() (*gorm.DB, map[string]*gorm.DB) {
	...
	//开启casbin的rbac权限
	if viper.GetBool("rbac.is_open") {
		// mysql适配器
		adapter, _ := gormadapter.NewAdapterByDB(dbList[viper.GetString("rbac.gorm")])
		if Enforcer, err = casbin.NewEnforcer("./config/casbin_rbac_model.conf", adapter); err != nil {
			log.Panic(errors.New("rbac配置有误：" + err.Error()))
		}
	}
